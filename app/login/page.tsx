...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (state.status === 'success') {
      ...
      // Honor `?next=` set by the middleware redirect — only same-origin paths.
      const next = searchParams.get('next');
      const target = next && next.startsWith('/') && !next.startsWith('//') ? next : '/dashboard';
      router.push(target);
    }
  }, [state, setSession, setMenus, router, searchParams]);
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center px-4 overflow-hidden">
      ...
      >
        <div className="relative rounded-3xl bg-white/60 dark:bg-white/5 backdrop-blur-2xl border border-zinc-200 dark:border-white/10 shadow-2xl shadow-indigo-500/20 overflow-hidden">
          <div
            aria-hidden
