...
import type {
  ...
  DepartmentUnitCreateBody, DepartmentUnitUpdateBody,
...
interface UnitModalProps {
  open:        boolean;
  onClose:     () => void;
  onSave:      (departmentId: string, body: DepartmentUnitCreateBody | DepartmentUnitUpdateBody) => Promise<void>;
...
export function UnitModal({ open, onClose, onSave, unit, departments }: UnitModalProps) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: { departmentId: '', code: '', name: '', description: '' },
  });
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      if (isEdit) {
        await onSave(unit!.departmentId, {
          name:        values.name,
          description: values.description || null,
        } satisfies DepartmentUnitUpdateBody);
      } else {
        await onSave(values.departmentId, {
          code:        values.code.toUpperCase(),
          name:        values.name,
          description: values.description || undefined,
        } satisfies DepartmentUnitCreateBody);
      }
    } catch (err: unknown) {
      const e = err as { code?: string };
      if (e.code === 'CODE_TAKEN') {
        form.setError('code', { message: 'This code is already taken in this department' });
      } else if (e.code === 'DEPARTMENT_NOT_FOUND') {
        form.setError('departmentId', { message: 'Department not found' });
            ...
            >
              ...
              <div className="flex items-center justify-between border-b border-border/60 px-5 py-4">
                <div className="flex items-center gap-3">
                  <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-linear-to-br from-violet-500 to-fuchsia-500 shadow-md shadow-violet-500/30">
                    <Layers className="h-4 w-4 text-white" />
                  </div>
                  <div>
                    <h2 className="text-[14px] font-bold text-foreground">
                      {isEdit ? 'Edit Unit' : 'Add Department Unit'}
                    </h2>
                    <p className="text-[11px] text-muted-foreground">
                      {isEdit ? `Editing ${unit?.name}` : 'Create a new department unit'}
                    </p>
                  </div>
                </div>
                <button
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)}>
                  <div className="space-y-4 px-5 py-5">
                    ...
                    {/* Department — create: selector / edit: readonly text */}
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>
                            Department {!isEdit && <span className="text-destructive">*</span>}
                          </FormLabel>
                          {isEdit ? (
